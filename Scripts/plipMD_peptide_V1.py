...
def plipmd(topol=None,traj=None):
	...
	peptide_chain=input ('\n\n1) Type the Chain of your peptide:\n>')
	...
	System=u.select_atoms('protein or (around 7 segid {})'.format(peptide_chain),updating=True)
	...
	for i in range(start,finish):
		...
		for ts in u.trajectory[i:i+1]:
			PDB.write(System)
			cmd.load(name,format='pdb')
			cmd.alter('chain {}'.format(peptide_chain), 'type="HETATM"')
			cmd.save(name,format='pdb')
			plip_job = PDBComplex()
			plip_job.load_pdb(name) 
			plip_job.analyze()
			for key in plip_job.interaction_sets:
				if key.split(':')[1]==peptide_chain:
					for interaction in plip_job.interaction_sets.get(key).all_itypes:
						if interaction.reschain != peptide_chain:
